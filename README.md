...
**v6.1 · Master + embedded/extra workers · GridFS + approval-based access, built for the Render.com free tier.**
...
> 🔐 **Zero-config credentials.** `config.py` is the single source of truth: the
> project's Telegram API ID/hash, bot token, owner ID and MongoDB Atlas URI are built
> in. Any environment variable still overrides the built-in value.
...
- `config.py` centralises built-in credentials, environment overrides and deployment constants.
...
|----------------------|---------|---------------------------------------------------------------|
| `API_ID` / `API_HASH` / `BOT_TOKEN` | built-in | Telegram credentials (override the defaults from `config.py`) |
| `OWNER_ID`           | built-in | Permanent owner (approves requests, manages admins)           |
