...
**v6.7 · Master + embedded/extra workers · GridFS + approval-based access, built for the Render.com free tier.**
...
- **Optional backup group** – each job gets its own forum topic with a *New Job* card, every delivered part and a completion / cancel / failure summary — posted by whichever worker (embedded or standalone) ran the job (set `BACKUP_GROUP_ID=0` to disable)
...
| Worker parity (v6.7) | The `worker.py` pipeline now matches everything the legacy in-process runner did: **output-aware splitting** (parts are sized with `EXPANSION[lang]` — a 500 KB split of a Hindi job therefore yields ~500 KB *Hindi* files instead of ~1.3 MB ones; Chinese/Japanese get fewer, fuller parts), the **backup group** is fed by the worker itself (forum topic per job → New Job card → every part → ✅/🛑/⚠️ summary, all best-effort so the group can never break a user's job), delivered file names are sanitised (`My/Novel: "Vol<1>"` → `MyNovel Vol1_hi_part01.txt`) and carry a part title + size caption, and **untranslated chunks** (all retries exhausted → kept in the source language) are counted and reported in the completion message, the queue document's `progress.failed_chunks`, the history row (`failed_chunks`) and the Mini App history. |
