...
7b. Statistics (v6.6): the worker's `_finish()` writes the history row (keys `uid, job_id, name, lang, fmt, split, size, parts, chars, secs, status, user, worker, ts[, error]`) and calls `MongoDatabase.bump_stats()` — an atomic `$inc` of `users.stats.{jobs,parts,chars}` and `stats.global.{jobs,parts,chars|failed|cancelled}`; only `done` jobs are credited. The master's `Store` never `replace_one`s user docs (`_save_user` uses `$set` without `stats`), and its RAM cache is refreshed by `Store.sync_from_db()` (rows with `ts > last synced`, then re-reads counters for the touched users + global; throttled to 2 s; forced from the janitor and the embedded worker's `on_finished` hook `_embedded_job_finished`, which also mirrors the row via `Store.ingest_finished()` and posts the backup-group summary). Legacy in-process `Store.bump()` uses the same `$inc` layout. Tests: `tests/test_store_sync.py`, `tests/test_worker_offline.py`.
7c. Worker parity (v6.7): `worker.py` splits with `split_factor(lang)` = `EXPANSION.get(lang, 1.15)` so delivered parts respect the user's split size; `safe_filename()` sanitises the title *before* taking the stem; part files are `<stem>_<lang>_partNN.<fmt>` (or `<stem>_<lang>.<fmt>` for a single part) with a `📘 name / 📦 Part i / n · lang / 💾 size` caption. Backup group (`TranslationWorker.backup_group`, default `BACKUP_GROUP_ID`): `_backup_topic()` creates a forum topic per job (falls back to the group root when topics are disabled), `_backup_text()` / `_backup_document()` post the New Job card, each delivered part and the done/cancelled/failed summary — every call is best-effort and never raises into the job. `_embedded_job_finished` in `bot.py` no longer posts a duplicate summary. `engine.failed` is summed into `failed_chunks`, written to `progress.failed_chunks`, the history row (`failed_chunks`, only when > 0) and shown in the completion message / Mini App history row.
